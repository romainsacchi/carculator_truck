...
class ExportInventory:
    ...
    def write_lci(self, presamples, ecoinvent_compatibility, ecoinvent_version, forbidden_activities=None):
        ...
        for d in dup:
            ...
            for row, col in coords[coords[:, 1] == d]:
                ...
                if "transport, heavy duty truck" in tuple_output[0]:
                    source = self.references["transport, heavy duty"]["source"]
                    description = self.references["transport, heavy duty"]["description"]
                    special_remark = self.references["transport, heavy duty"]["special remark"]

                elif "transport, medium duty truck" in tuple_output[0]:
                    source = self.references["transport, medium duty"]["source"]
                    description = self.references["transport, medium duty"]["description"]
                    special_remark = self.references["transport, medium duty"]["special remark"]

                elif "Heavy duty truck" in tuple_output[0]:
                    source = self.references["heavy duty"]["source"]
                    description = self.references["heavy duty"]["description"]
                    special_remark = self.references["heavy duty"]["special remark"]

                elif "Medium duty truck" in tuple_output[0]:
                    source = self.references["medium duty"]["source"]
                    description = self.references["medium duty"]["description"]
                    special_remark = self.references["medium duty"]["special remark"]

                else:
                    source = self.references[tuple_output[0]]["source"] if tuple_output[0] in self.references else ""
                    description = self.references[tuple_output[0]]["description"] if tuple_output[
                                                                                         0] in self.references else ""
                    special_remark = self.references[tuple_output[0]]["special remark"] if tuple_output[
                                                                                               0] in self.references else ""
                                    ...
                                    if ecoinvent_version == "uvek":
                                        if not any(i.lower() in e["name"].lower()
                                            ...
                                            uvek_multiplication_factors = {
                                                ...
                                                "market for manual dismantling of used electric passenger car": 1 / 1600,
    ...
    def write_lci_to_bw(self, presamples, ecoinvent_compatibility, ecoinvent_version, forbidden_activities=None):
        """
